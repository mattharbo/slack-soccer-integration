...
foreach ($gamelist as $fixtureretrieve) {
	foreach ($fixtureretrieve as $gameretrieve) {

		//Loop to parse back up game(s)
		foreach ($gamesinbackup as $fixtureinbackup) {
			foreach ($fixtureinbackup as $gamebu) {

				if ($gamebu['homeTeamName'] == $gameretrieve['homeTeamName']) {
					echo "same home team !";
				}else{
					echo "There is a new game available in realtime data feed (vs. backup)<br>";
				}
